...
use crate::metadata::{
    AstQuery, Symbol, SymbolTableEdit, SymbolTableEditor, SymbolTableQuery, Visitable, SymbolTableManager,
};
...
pub fn rename(
    ...
    if let Some(symbol) = symbol_exist{
        ...
        return_changes = Some(WorkspaceEdit::new(build_changes(cur_url, &symbol, new_name,graph)))
    }
    return_changes
...
fn build_changes(curr: Url, symbol: &Symbol, new_name: String,graph:&FileGraph) -> HashMap<Url, Vec<TextEdit>> {
    ...
    for usage in symbol.get_usages() {
        info!("Usage: {:?}",usage);
        let url =graph.find_url_from_node_index(usage.file_id.unwrap()).unwrap();
        if let Some(existing_vector) = map.get_mut(&url) {
            existing_vector.push(TextEdit::new(usage.range.clone(), new_name.clone()));
        } else {
            let new_vector = vec![TextEdit::new(usage.range.clone(), new_name.clone())];
